...
Standardized discount formats using Excel functions (TEXT TO COLUMN, IF, ISNUMBER, ISBLANK).

Handled missing values with conditional formulas and category-based averages.

Replaced null size options with "Not Available".

Data Analysis

Calculated average original price for products with ratings > 4.

Counted products with discounts greater than 50%.

Identified inventory distribution by size (e.g., size M as most common).

Classified products into High Discount vs. Low Discount categories.

Data Retrieval & Lookup

Applied VLOOKUP/XLOOKUP to fetch brand, price, and rating by product ID.

Used INDEX-MATCH for precise discount price retrieval.
